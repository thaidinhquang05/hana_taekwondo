...
async function getEarningValue(month, year) {
	...
	await $.ajax({
		...
		success: (res) => {
			...
			$(".earning-monthly").append("VND " + number_format(res.data.monthly));

			$(".earning-annual").empty();
			$(".earning-annual").append("VND " + number_format(res.data.annual));
...
async function getSpendingValue(month, year) {
	...
	await $.ajax({
		...
		success: (res) => {
			...
			$(".spending-monthly").append(`VND ${number_format(data.monthly.total)}`);

			$(".spending-annual").empty();
			$(".spending-annual").append(`VND ${number_format(data.spendingAnnual)}`);
		},
		error: (xhr) => {
			$.toast({
				heading: "Error!!!",
