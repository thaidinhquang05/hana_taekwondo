                    ...
                    row.innerHTML = `
                        <td class="border-less">
                            <img class="img-profile rounded-circle"
                                src="${
                                    notification.studentImg
                                }" style="width: 50px; height: 50px;" />
								</td>
								<td class="border-less">
                            <span>We would like to remind you that the final 
							deadline for paying <strong style="color: red;">${
                                notification.fullName
                            }</strong>'s course fees is approaching. There are only <strong style="color: red;">${getNotificationDueDate(
                        notification.dueDate
                    )}</strong> days left.
							 Please make sure to make the payment on time to avoid any interruptions in <strong style="color: red;">${
                                 notification.fullName
                             }</strong> learning journey.</span>
							 </td>
							 <td>
							 <span class="notification-time" style="float: right;">${getNotificationTime(
...
function getNotificationDueDate(notificationTime) {
    ...
    const currentTime = new Date();
    var elapsedMinutes = Math.floor((notification - currentTime) / 60000);
