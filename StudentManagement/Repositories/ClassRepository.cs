...
public class ClassRepository : Repository<Class>, IClassRepository
{
    ...
    {
        var _class = _context.Classes.FirstOrDefault(s => s.Id == _classId);

        foreach (var item in _studentIds)
        {
            var _student = _context.Students.FirstOrDefault(s => s.Id == item);
    ...
    public void RemoveStudentFromClass(int _studentId, int _classId)
    {
        var studentClass = _context.StudentClasses
            .FirstOrDefault(sc => sc.StudentId == _studentId && sc.ClassId == _classId);
    ...
    {
        var result = _context.Classes.FirstOrDefault(c => c.Id == classId);
        return result;
    }
    ...
    public List<StudentAttendanceOutput> GetStudentBySlotAndDate(int slotId, DateTime date)
    {
        ...
        {
            var student = _context.StudentTimetables
                .Where(st => st.TimeTableId == timetable.Id)
        ...
        foreach (var item in students)
        {
            ...
            var attend = _context.Attendances
                .FirstOrDefault(a => a.Date == date && a.SlotId == slotId && a.StudentId == item.Id);
            ...
            if (item != null)
            {
                if (item.PaidDate <= date && date <= item.DueDate)
                {
                    var student = _context.Students
                        .Where(s =>
                            s.Attendances.Any(a => a.IsAttendance == false && a.Date < date))
                        .FirstOrDefault(s => s.Id == item.StudentId);
                    if (student != null)
                    {
                        StudentAttendanceOutput studentAttendanceOutput =
                            new StudentAttendanceOutput();
                        studentAttendanceOutput.Index = index;
    ...
    public void TakeAttendance(int slotId, DateTime date,
        List<StudentAttendanceInput> studentAttendanceInputs)
    {
        foreach (var item in studentAttendanceInputs)
        {
            var attendance = _context.Attendances
                .FirstOrDefault(a =>
                    a.SlotId == slotId && a.Date == date && a.StudentId == item.Id);
            ...
            if (attendance == null)
            {
                ...
                newAttendance.Student =
                    _context.Students.Where(s => s.Id == item.Id).FirstOrDefault();
    ...
    public void TakeMakeUpAttendance(int slotId, DateTime date,
        List<StudentAttendanceInput> studentAttendanceInputs)
    {
        foreach (var item in studentAttendanceInputs)
        {
            var tution = _context.Tuitions.Where(t => t.StudentId == item.Id)
                .OrderByDescending(t => t.DueDate).FirstOrDefault();
            var attendance = _context.Attendances.Include(a =>
                a.Student).FirstOrDefault(a => a.IsAttendance == false
                                               && a.StudentId == item.Id
